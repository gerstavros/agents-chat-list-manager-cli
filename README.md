...
A terminal / SSH-friendly app for managing Claude Code, Qwen Code, and codewhale-tui conversation history not only
...
| `t` | cycle the tool filter (All → claude_code → qwen_code → codewhale_tui → All) |
| `e` | export the selected conversation (prompts for output path) |
| `r` | rescan |
| `q` | quit (in the transcript view, `q` or `←` goes back to the list) |

Same `--claude-dir`/`--qwen-dir`/`--codewhale-dir` one-off overrides work here
...
```
--claude-dir PATH  --qwen-dir PATH  --codewhale-dir PATH
```
...
against fixture files under `tests/fixtures/` — no GUI project checkout and
no real `~/.claude` / `~/.qwen` / `~/.codewhale` data required. It mirrors
